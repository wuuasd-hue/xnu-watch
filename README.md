...
2. **Secrets ekle** — Repo → Settings → Secrets and variables → Actions →
   ...
   - `GROQ_API_KEY` — console.groq.com (ücretsiz, kart istemiyor)
   - `TOGETHER_API_KEY` — together.ai (ücretsiz tier)
...
Tamamen ücretsiz tier'lar üzerinde çalışır (Groq + Together.ai). Her aday için
6 model × 3 katman = 18 API çağrısı yapılır; `MAX_CANDIDATES_PER_RUN` ile
çalıştırma başına işlenen aday sayısı sınırlanır (varsayılan: 4).
